...
def evaluate_pitchers(season_stats: pd.DataFrame,
                      stuff_metrics: pd.DataFrame,
                      location: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    박스스코어 시즌 성적(ERA/FIP)과 kbostuff 구위·로케이션 지표를
    ...
    location      : kbostuff_client.summarize_location() 결과 (선택)
                    K-Location+, edge/heart/waste 를 붙여 Gap 축 진단에 씁니다.
    """
    ...
    if location is not None and not location.empty:
        df = df.merge(
            location.rename(columns={"pitcher_pcode": "pcode"}),
            on="pcode", how="left",
        )
    for c in ("k_location", "edge_pct", "heart_pct", "waste_pct"):
    ...
    # ── Gap 축 (구위 − 로케이션) ──
    # kbostuff가 2축으로 내세우는 값입니다.
    #   Gap ≈ 0  : 구위·제구 균형
    #   Gap  +   : 파워형 — 공은 좋은데 제구가 흔들림
    #   Gap  −   : 로케이션형 — 구위는 평범해도 위치로 승부
    # '억울한 투수'(구위 좋고 성적 나쁨)의 원인을 갈라줍니다:
    #   Gap이 크면 제구 교정 이슈(고칠 수 있음), 작으면 순수 불운.
    df["stuff_loc_gap"] = df["k_stuff_v2"] - df["k_location"]

    def loc_note(row) -> str:
        g = row["stuff_loc_gap"]
        if pd.isna(g):
            return "➖ 로케이션 정보 없음"
        if g >= 5:
            return "🎯 파워형 (구위 우위, 제구 교정 여지)"
        if g <= -5:
            return "🧵 로케이션형 (위치로 버티는 유형)"
        return "⚖️ 균형형"

    df["loc_type"] = df.apply(loc_note, axis=1)
    ...
    cols = ["pcode", "name", "team", "team_name", "ip", "era", "fip",
            ...
            "kk", "bb", "hr", "type",
            # 로케이션(Gap 축) — 이미 받던 pitch_type_details에서 뽑은 값
            "k_location", "stuff_loc_gap", "loc_type",
            "edge_pct", "heart_pct", "waste_pct", "reliability",
...
def pitcher_screens(evaluated: pd.DataFrame) -> dict:
    ...
    # 🎯 억울한 투수 중 '제구가 원인'인 유형 — 고칠 수 있는 반등 후보.
    #   순수 불운(균형형)과 달리 제구 교정으로 성적이 따라올 여지가 있습니다.
    fixable = unlucky[unlucky["stuff_loc_gap"] >= 5] \
        .sort_values("stuff_loc_gap", ascending=False)

    # ⚠️ 한가운데 실투가 잦은 투수 — 아직 안 터졌어도 피장타 폭발 예비군.
    #   heart_pct(리그 평균 ≈ 0.11)가 높을수록 위험합니다.
    meatball = evaluated.dropna(subset=["heart_pct"])
    meatball = meatball[meatball["heart_pct"] >= 0.14] \
        .sort_values("heart_pct", ascending=False)

    return {
        "unlucky": unlucky,
        "timebomb": timebomb,
        "defense_victim": defense_victim,
        "fixable": fixable,       # 🎯 제구만 잡으면 반등
        "meatball": meatball,     # ⚠️ 한가운데 실투 경계
    }
...
def evaluate_batters(bat_metrics: pd.DataFrame,
    ...
    # → 세이버매트릭스에서 가장 오래되고 검증된 '운 탐지기'입니다.
    #
