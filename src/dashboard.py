...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .brk{width:22px;text-align:center;color:var(--muted);font-weight:800;font-size:13px;
       font-variant-numeric:tabular-nums;}
  .blg{height:17px;}
...
let DATA=null, dates=[], N=0, cur=null, t=0, playing=false, raf=null, boardRows={};
const ROWH=31;
...
// ── 동적 순위표(재생 중 순위가 슬라이드하며 바뀜) ──
function buildBoard(){
  const ls=DATA.races[cur].leaders; boardRows={};
  let h='';
  ls.forEach(p=>{
  ...
  ls.forEach(p=> boardRows[p.pcode]=b.querySelector('[data-pc="'+p.pcode+'"]'));
  boardUpdate(t);
}
function boardUpdate(ti){
  const ls=DATA.races[cur].leaders; if(!ls||!Object.keys(boardRows).length) return;
  const vals=ls.map(p=>({p, v:(p.series&&p.series[ti]!=null)?p.series[ti]:0}));
...
def _team_style_card(logos: dict, rank_order: list | None = None,
    ...
    for rk, i in enumerate(order, 1):
        ...
        body.append(
            ...
            f'<td>{_style_gauge(ax3[i], t3)}</td>{hacell}{pcell}</tr>')
    ...
    ha_note = (' ⑤ <b>홈/원정</b>: 홈 승률 − 원정 승률 (왼쪽=원정 강, 오른쪽=안방 강).'
               if ha_ax is not None else '')
    return (
        '<div class="card wide">'
...
def _one_game_card(g, logos) -> str:
    ...
    if True:
        ...
        return (
            ...
            f'<span style="display:inline-flex;align-items:center;gap:4px;font-size:13px;white-space:nowrap">'
            f'<img src="{al}" alt="" style="width:18px;height:18px;object-fit:contain;flex:none">{g["awayName"]}</span>'
            ...
            f'<span style="display:inline-flex;align-items:center;gap:4px;font-size:13px;white-space:nowrap">'
            f'{g["homeName"]}<img src="{hl}" alt="" style="width:18px;height:18px;object-fit:contain;flex:none"></span></div>'
...
(function () {
  ...
  if (!b) return;
  // 로컬 서버(localhost)가 아니면(배포 사이트·아이패드 등) 버튼 숨김 — 어차피 작동 안 함
  if (!["localhost", "127.0.0.1"].includes(location.hostname)) { b.style.display = "none"; return; }
  b.onclick = async () => {
    try {
      b.disabled = true;
